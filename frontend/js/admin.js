...
async function loadCourses() {
  try {
    const res = await adminFetch(`${ADMIN_API}/courses`);
    if (!res) return;
    courses = await res.json();
  } catch(e) {
    // fallback to localStorage if server unreachable
    courses = JSON.parse(localStorage.getItem('sharedCourses') || '[]');
  }
  syncAdminCoursesToDataStore(courses);
...
function initAddCourseForm() {
  ...
  btn.addEventListener('click', () => {
    ...
    adminFetch(`${ADMIN_API}/courses`, {
      method: 'POST',
      body: JSON.stringify({ code, name, instructor, term, credits, description, categories }),
    }).then(async res => {
      if (!res) return;
      if (!res.ok) { const d = await res.json(); showToast(d.error || 'Error adding course.', 'error'); return; }
...
function openEditCourse(id) {
  ...
  modal.querySelector('#editSaveBtn').onclick = async () => {
    const body = {
      code:        modal.querySelector('#editCode').value.trim(),
      name:        modal.querySelector('#editName').value.trim(),
      instructor:  modal.querySelector('#editInstr').value.trim(),
      term:        modal.querySelector('#editTerm').value,
      credits:     parseInt(modal.querySelector('#editCredits').value) || 3,
      description: modal.querySelector('#editDesc').value.trim(),
    };
    const res = await adminFetch(`${ADMIN_API}/courses/${course._id}`, { method: 'PUT', body: JSON.stringify(body) });
    if (!res) return;
    if (!res.ok) { const d = await res.json(); showToast(d.error || 'Error updating.', 'error'); return; }
...
async function deleteCourse(id) {
  ...
  const res = await adminFetch(`${ADMIN_API}/courses/${id}`, { method: 'DELETE' });
  if (!res) return;
  if (!res.ok) { const d = await res.json(); showToast(d.error || 'Error deleting.', 'error'); return; }
...
async function toggleCourse(id) {
  const res = await adminFetch(`${ADMIN_API}/courses/${id}/toggle`, { method: 'PATCH' });
  if (!res) return;
  const data = await res.json();
  showToast(`Course ${data.enabled ? 'enabled — now visible to students' : 'disabled — hidden from students'}.`);
  await loadCourses();
}
...
function initAssessmentSection() {
  ...
  addBtn.addEventListener('click', () => {
    ...
    adminFetch(`${ADMIN_API}/courses/${courseId}/categories`, {
      method: 'POST',
      body: JSON.stringify({ title: name, type, description: desc, dueDate, weight, totalMarks, status }),
    }).then(async res => {
      if (!res) return;
      if (!res.ok) { const d = await res.json(); showToast(d.error || 'Error adding assessment.', 'error'); return; }
      showToast('Assessment added!');
      clearAssessmentForm();
...
async function loadStats() {
  try {
    const res = await adminFetch(`${ADMIN_API}/stats`);
    if (!res) return;
    statsCache = await res.json();
  } catch(e) {
    statsCache = typeof adminGetStats === 'function' ? adminGetStats() : [];
  }
  populateStatsSelect();
}
